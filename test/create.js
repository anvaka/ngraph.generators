...
test('cliqueCircle graph', function(t) {
  var graph = generators.cliqueCircle(2, 10);
  t.equal(graph.getNodesCount(), 20, 'Unexpected number of nodes for clique graph');
  t.equal(graph.getLinksCount(), 92, 'Unexpected number of links for clique graph');
  t.end();
});
...
test('20 nodes, 100% rebuild', function (t) {
  var graph = generators.wattsStrogatz(10, 9, 1, 1);
  t.equal(graph.getNodesCount(), 10, 'Unexpected number of nodes for Watts-Strogatz graph n=10, k=9, p=1');
  t.equal(graph.getLinksCount(), 40, 'Unexpected number of links for Watts-Strogatz graph n=10, k=9, p=1');
  t.end();
});
